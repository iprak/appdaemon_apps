...
class MyPointsTest(hass.Hass):
    def initialize(self):
        self.servers = self.args["servers"]
        for item in self.servers:
            self.log(
                f'{item["server"]} {item["email"]} {item["passwd"]} {item["folder"]}'
            )
    ...
    def check(self, kwargs=None):
        for item in self.servers:
            self.check_emails(
                item["server"], item["email"], item["passwd"], item["folder"]
            )
    ...
    def check_emails(self, server, email, passwd, folder):
        ...
        try:
            ...
            self.log(f"{server}: Unable to connect {error}")
            return

        try:
            client.login(email, passwd)
        except Exception as error:
            self.log(f"{server}: IMAPClient login error {error}")
            return
        ...
        # emails = []
        unique_links = {}
        #self.log(f"{server}: Connected getting messages with flag {flag}")

        today = datetime.today()
        cutoff = today - timedelta(days=1)

        try:
            messages = client.search(["SINCE", cutoff.strftime("%d-%b-%Y")])

            message_items = client.fetch(messages, "RFC822").items()
            self.log(f"{server}: {len(message_items)} messages")
